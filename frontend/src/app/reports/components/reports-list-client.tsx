...
export function ReportsListClient({ savedReports }: ReportsListClientProps) {
  ...
  const handleDownload = useCallback((report: SavedReport) => {
    if (report.downloadUrl && report.downloadUrl !== "#") {
      try {
        ...
        toast.error(`Erreur de téléchargement. Impossible de télécharger le rapport "${report.name}".`);
      }
    } else {
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      toast.error(`Erreur de suppression. Impossible de supprimer le rapport "${reportToDelete.name}".`);
    } finally {
